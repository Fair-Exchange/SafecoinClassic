...
bool AuctionExactAmounts(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx,int32_t minage,uint64_t txfee)
{
    ...
    for (auto txvin : tx.vin)
    {
        ...
        if ((*cp->ismyvin)(txvin.scriptSig))
        {
            ...
            if ( (assetoshis= IsAuctionvout(cp,vinTx,txvin.prevout.n)) != 0 )
                inputs += assetoshis;
        }
    }
...
bool AuctionValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    //     if ( !AuctionExactAmounts(cp,eval,tx,1,10000) )
    //     {
    ...
    //         else
    //             fprintf(stderr,"Auctionget invalid\n");
    //         return retval;
    //     }
    // }
}
...
int64_t AddAuctionInputs(struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey pk,int64_t total,int32_t maxinputs)
{
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        if ( unspentOutput.second.satoshis < 1000000 )
            continue;
        if ( GetTransaction(txid,vintx,hashBlock,false) && (nValue= IsAuctionvout(cp,vintx,(int32_t)unspentOutput.first.index)) > 0 ) {
...
std::string AuctionBid(uint64_t txfee,uint256 itemhash,int64_t amount)
{
    ...
    if ( txfee == 0 )
        txfee = 10000;
    Auctionpk = GetUnspendable(cp, nullptr);
    ...
    fprintf(stderr,"cant find Auction inputs\n");
    return "";
}
