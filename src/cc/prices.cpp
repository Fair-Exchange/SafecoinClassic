...
int64_t PricesOraclePrice(int64_t &rektprice,uint64_t mode,uint256 oracletxid,std::vector<CPubKey>pubkeys,int32_t dir,int64_t amount,int32_t leverage)
{
    ...
    if ( dir > 0 )
        ...
        rektprice = price * (leverage-1) / leverage;
    return price;
}
...
bool PricesValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    //else
    {
        ...
        if (retval)
            ...
            fprintf(stderr,"Pricesget invalid\n");
        return retval;
    }
}
...
int64_t AddTokensInputs(struct CCcontract_info *cp,CMutableTransaction &mtx,char *destaddr,uint256 tolenid,int64_t total,int32_t maxinputs)
{
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        if (GetTransaction(txid,vintx,hashBlock,false) && vout < vintx.vout.size())
        {
            ...
            if ((nValue= vintx.vout[vout].nValue) >= 10000 && !myIsutxo_spentinmempool(txid,vout))
            {
...
UniValue PricesList()
{
    ...
    for (auto address : addressIndex)
    {
        ...
        if (GetTransaction(txid,vintx,hashBlock,false) && !vintx.vout.empty() && DecodePricesFundingOpRet(vintx.vout.back().scriptPubKey,planpk,oracletxid,longtoken,shorttoken,margin,mode,maxleverage,pubkeys,bettoken) == 'F')
...
std::string PricesCreateFunding(uint64_t txfee,uint256 bettoken,uint256 oracletxid,uint64_t margin,uint64_t mode,uint256 longtoken,uint256 shorttoken,int32_t maxleverage,int64_t funding,std::vector<CPubKey> pubkeys)
{
    ...
    CTransaction oracletx; int64_t fullsupply,inputs,CCchange=0; uint256 hashBlock; char str[65],coinaddr[64],houseaddr[64]; CPubKey mypk,pricespk; int32_t i,N; struct CCcontract_info *cp,C;
    ...
    if (!GetCCaddress1of2(cp,houseaddr,pricespk,mypk))
    {
    ...
    if (!GetTransaction(oracletxid,oracletx,hashBlock,false) || oracletx.vout.empty())
    {
...
{
    UniValue result(UniValue::VOBJ),a(UniValue::VARR); CPubKey pricespk,planpk; uint256 hashBlock,oracletxid,longtoken,shorttoken,bettoken; CTransaction vintx; int64_t balance,supply,exposure; uint64_t funding,mode; int32_t margin,maxleverage; char numstr[65],houseaddr[64],exposureaddr[64],str[65]; std::vector<CPubKey>pubkeys; struct CCcontract_info *cp,C;
    cp = CCinit(&C,EVAL_PRICES);
    pricespk = GetUnspendable(cp, nullptr);
    if (!GetTransaction(fundingtxid,vintx,hashBlock,false))
    {
        fprintf(stderr,"cant find fundingtxid\n");
        ERR_RESULT("cant find fundingtxid");
        return result;
    }
    if (!vintx.vout.empty() && DecodePricesFundingOpRet(vintx.vout.back().scriptPubKey,planpk,oracletxid,longtoken,shorttoken,margin,mode,maxleverage,pubkeys,bettoken) == 'F')
    {
        ...
        for (auto pubkey : pubkeys)
            a.push_back(pubkey33_str(str,(uint8_t *)&pubkey));
...
std::string PricesAddFunding(uint64_t txfee,uint256 refbettoken,uint256 fundingtxid,int64_t amount)
{
    ...
    if (!GetTransaction(fundingtxid,tx,hashBlock,false))
    {
        fprintf(stderr,"cant find fundingtxid\n");
        return "";
    }
    if (!tx.vout.empty() && DecodePricesFundingOpRet(tx.vout.back().scriptPubKey,planpk,oracletxid,longtoken,shorttoken,margin,mode,maxleverage,pubkeys,bettoken) == 'F' && bettoken == refbettoken)
    {
        GetCCaddress1of2(cp,houseaddr,pricespk,planpk);
...
std::string PricesBet(uint64_t txfee,uint256 refbettoken,uint256 fundingtxid,int64_t amount,int32_t leverage)
{
    ...
    if (!tx.vout.empty() && DecodePricesFundingOpRet(tx.vout.back().scriptPubKey,planpk,oracletxid,longtoken,shorttoken,margin,mode,maxleverage,pubkeys,bettoken) == 'F' && bettoken == refbettoken)
    {
        ...
        tokenid = dir < 0 ? shorttoken : longtoken;
