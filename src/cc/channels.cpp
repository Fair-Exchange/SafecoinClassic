...
int64_t IsChannelsvout(struct CCcontract_info *cp,const CTransaction& tx,CPubKey srcpub, CPubKey destpub,int32_t v)
{
    ...
    if (tx.vout[v].scriptPubKey.IsPayToCryptoCondition())
        if (Getscriptaddress(destaddr,tx.vout[v].scriptPubKey) && strcmp(destaddr,channeladdr) == 0)
...
int64_t IsChannelsMarkervout(struct CCcontract_info *cp,const CTransaction& tx,CPubKey srcpub,int32_t v)
{
    ...
    if (tx.vout[v].scriptPubKey.IsPayToCryptoCondition())
        if (Getscriptaddress(destaddr,tx.vout[v].scriptPubKey) && strcmp(destaddr,ccaddr) == 0)
            return tx.vout[v].nValue;
    return 0;
}
...
bool ChannelsExactAmounts(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx,int32_t minage,uint64_t txfee)
{
    ...
    CTransaction vinTx; uint256 hashBlock; int32_t i; int64_t inputs=0,outputs=0,assetoshis;

    if (tx.vout.empty() || DecodeChannelsOpRet(tx.vout.back().scriptPubKey, txid, srcpub, destpub, param1, param2, param3) == 0)
    ...
    for (auto txvin : tx.vin) {
        ...
        inputs += vinTx.vout[txvin.prevout.n].nValue;
    }
    for (auto txvout : tx.vout)
        outputs += txvout.nValue;
...
{
    int32_t preventCCvins,preventCCvouts,i,numpayments,p1,param1; bool retval;
    ...
    if (!ChannelsExactAmounts(cp,eval,tx,1,10000))
    {
    ...
    if ( (funcid = DecodeChannelsOpRet(tx.vout.back().scriptPubKey, opentxid, srcpub, destpub, param1, param2, param3)) != 0)
    {
        switch ( funcid )
        {
            case 'O':
            ...
            case 'P':
                ...
                if (!safecoin_txnotarizedconfirmed(opentxid))
                ...
                if (!IsCCInput(tx.vin[1].scriptSig))
                    return eval->Invalid("vin.1 is CC for channelPayment!");
                if (!IsCCInput(tx.vin[2].scriptSig))
                ...
                if (tx.vout[3].scriptPubKey.IsPayToCryptoCondition())
                    return eval->Invalid("vout.3 is normal for channelPayment!");
                if (tx.vout[3].scriptPubKey!=CScript() << ParseHex(HexStr(destpub)) << OP_CHECKSIG)
                    return eval->Invalid("payment funds do not go to receiver!");
                if ( param1 > CHANNELS_MAXPAYMENTS)
                    return eval->Invalid("too many payment increments!");
                if (myGetTransaction(opentxid,channelOpenTx,hashblock)) {
                    if (!channelOpenTx.vout.empty() && (funcid=DecodeChannelsOpRet(channelOpenTx.vout.back().scriptPubKey, tmp_txid, srcpub, destpub, numpayments, payment, hashchain)) != 0 && funcid!='O')
                    ...
                    if (tx.vout[3].nValue != param2*payment)
                        return eval->Invalid("vout amount does not match number_of_payments*payment!");
                }
                if (myGetTransaction(tx.vin[1].prevout.hash,prevTx,hashblock)) {
                    if (!prevTx.vout.empty() && DecodeChannelsOpRet(prevTx.vout.back().scriptPubKey, tmp_txid, srcpub, destpub, p1, p2, p3) == 0)
                        return eval->Invalid("invalid previous tx OP_RETURN data!");
                    if (tx.vout[1].scriptPubKey != prevTx.vout[1].scriptPubKey)
                        return eval->Invalid("invalid destination for sender marker!");
                    if (tx.vout[2].scriptPubKey != prevTx.vout[2].scriptPubKey)
            ...
            case 'C':
                ...
                if (!channelOpenTx.vout.empty() && DecodeChannelsOpRet(channelOpenTx.vout.back().scriptPubKey, tmp_txid, srcpub, destpub, numpayments, payment, hashchain) != 'O')
                    return eval->Invalid("invalid channelopen OP_RETURN data!");
                if (tx.vout[0].nValue != param1*payment)
                    return eval->Invalid("vout amount does not match number_of_payments*payment!");
                if (myGetTransaction(tx.vin[1].prevout.hash,prevTx,hashblock))
                {
                    if (!prevTx.vout.empty() && DecodeChannelsOpRet(prevTx.vout.back().scriptPubKey, tmp_txid, srcpub, destpub, p1, p2, p3) == 0)
            ...
            case 'R':
                ...
                if (!safecoin_txnotarizedconfirmed(opentxid) )
                    return eval->Invalid("channelOpen is not yet confirmed(notarised)!");
                if (!safecoin_txnotarizedconfirmed(param3) )
                ...
                if ( tx.vout[2].scriptPubKey!=CScript() << ParseHex(HexStr(srcpub)) << OP_CHECKSIG)
                ...
                if (!channelOpenTx.vout.empty() && DecodeChannelsOpRet(channelOpenTx.vout.back().scriptPubKey, tmp_txid, srcpub, destpub, numpayments, payment, hashchain) != 'O')
                    return eval->Invalid("invalid channelopen OP_RETURN data!");
                if (!myGetTransaction(param3,channelCloseTx,hashblock))
                    return eval->Invalid("invalid close txid!");
                if (!channelCloseTx.vout.empty() && DecodeChannelsOpRet(channelCloseTx.vout.back().scriptPubKey, tmp_txid, srcpub, destpub, param1, param2, param3) != 'C')
                    return eval->Invalid("invalid channelclose OP_RETURN data!");
                if (tmp_txid != opentxid)
                    return eval->Invalid("invalid close tx, opentxid do not match on close and refund!");
                if (tx.vout[2].nValue != param1*payment)
                    return eval->Invalid("vout amount does not match number_of_payments*payment!");
                if (myGetTransaction(tx.vin[1].prevout.hash,prevTx,hashblock))
                {
                    if (!prevTx.vout.empty() && DecodeChannelsOpRet(prevTx.vout.back().scriptPubKey, tmp_txid, srcpub, destpub, p1, p2, p3) == 0)
                        return eval->Invalid("invalid previous tx OP_RETURN data!");
                    if (tx.vout[0].scriptPubKey != prevTx.vout[1].scriptPubKey)
                        return eval->Invalid("invalid destination for sender marker!");
                    if (tx.vout[1].scriptPubKey != prevTx.vout[2].scriptPubKey)
                        return eval->Invalid("invalid destination for receiver marker!");
                    if (tx.vout[2].nValue != prevTx.vout[0].nValue)
        ...
        return eval->Invalid("unexpected channels missing funcid");
    retval = PreventCC(eval,tx,preventCCvins,tx.vin.size(),preventCCvouts,tx.vout.size());
    if (retval)
        fprintf(stderr,"Channel tx validated\n");
    else
        fprintf(stderr,"Channel tx invalid\n");
    return retval;
}
...
int64_t AddChannelsInputs(struct CCcontract_info *cp,CMutableTransaction &mtx, CTransaction openTx, uint256 &prevtxid)
{
    ...
    if (openTx.vout.empty() || DecodeChannelsOpRet(openTx.vout.back().scriptPubKey,tmp_txid,srcpub,destpub,param1,param2,param3)!='O')
    {
    ...
    {
        if ( unspentOutput.first.index == 0 && GetTransaction(unspentOutput.first.txhash,tx,hashBlock,false) && !tx.vout.empty())
        {
            if (DecodeChannelsOpRet(tx.vout.back().scriptPubKey,tmp_txid,srcpub,destpub,param1,param2,param3) != 0 &&
    ...
    if (txid != zeroid && myIsutxo_spentinmempool(txid, 0))
    {
        txid = zeroid;
        ...
        for (const CTxMemPoolEntry &e : mempool.mapTx) {
            ...
            if (!txmempool.vout.empty() && DecodeChannelsOpRet(txmempool.vout.back().scriptPubKey,tmp_txid,srcpub,destpub,param1,param2,param3) != 0 &&
...
std::string ChannelPayment(uint64_t txfee,uint256 opentxid,int64_t amount, uint256 secret)
{
    ...
    struct CCcontract_info *cp,C; int32_t i,funcid,prevdepth,numpayments,totalnumpayments;
    ...
    if (AddNormalinputs(mtx,mypk,2*txfee,3) > 0)
    {
        ...
        {
            if (!channelOpenTx.vout.empty() && DecodeChannelsOpRet(channelOpenTx.vout.back().scriptPubKey, txid, srcpub, destpub, totalnumpayments, payment, hashchain)=='O')
            {
                ...
                if (amount % payment != 0 || amount<payment)
                {
                    fprintf(stderr,"invalid amount, not a magnitude of payment size\n");
                    return "";
                }
                numpayments=amount/payment;
                if (GetTransaction(prevtxid,prevTx,hashblock,false) && !prevTx.vout.empty() &&
                  ((funcid = DecodeChannelsOpRet(prevTx.vout.back().scriptPubKey, txid, srcpub, destpub, prevdepth, param2, param3)) != 0) &&
                  (funcid == 'P' || funcid=='O'))
                {
...
std::string ChannelClose(uint64_t txfee,uint256 opentxid)
{
    ...
    if ( txfee == 0 )
        txfee = 10000;
    mypk = pubkey2pk(Mypubkey());
    if (!GetTransaction(opentxid,channelOpenTx,hashblock,false))
    {
        fprintf(stderr, "invalid channel open txid\n");
        return "";
    }
...
std::string ChannelRefund(uint64_t txfee,uint256 opentxid,uint256 closetxid)
{
    ...
    if ( txfee == 0 )
        txfee = 10000;
    mypk = pubkey2pk(Mypubkey());
    if (!GetTransaction(closetxid,channelCloseTx,hashblock,false))
    {
        fprintf(stderr, "invalid channel close txid\n");
        return "";
    }
    if (channelCloseTx.vout.empty() || DecodeChannelsOpRet(channelCloseTx.vout.back().scriptPubKey,txid,srcpub,destpub,param1,param2,param3)!='C')
    {
    ...
    if (channelOpenTx.vout.empty() || DecodeChannelsOpRet(channelOpenTx.vout.back().scriptPubKey,txid,srcpub,destpub,numpayments,payment,hashchain)!='O')
    {
        fprintf(stderr, "invalid channel open tx\n");
        return "";
    }
    ...
    if ( AddNormalinputs(mtx,mypk,2*txfee,3) > 0 )
    {
        ...
        {
            if (GetTransaction(prevtxid,prevTx,hashblock,false) && !prevTx.vout.empty() &&
                DecodeChannelsOpRet(prevTx.vout.back().scriptPubKey, txid, srcpub, destpub, param1, param2, param3) != 0)
            {
                hentropy = DiceHashEntropy(entropy, channelOpenTx.vin[0].prevout.hash, channelOpenTx.vin[0].prevout.n,1);
                endiancpy(hash, (uint8_t * ) & hentropy, 32);
                for (i = 0; i < param1; i++)
...
{
    UniValue result(UniValue::VOBJ); CTransaction tx,opentx; uint256 txhash,tmp_txid,hashBlock,param3,opentxid,hashchain,prevtxhash;
    ...
    if (channeltxid==zeroid)
    {
        ...
        for (auto txid : txids)
        {
            ...
            txhash = txid.first.txhash;
            vout = (int32_t)txid.first.index;
            nValue = (int64_t)txid.second;
            if ((vout == 1 || vout == 2) && nValue == 10000 && GetTransaction(txhash,tx,hashBlock,false) && !tx.vout.empty() &&
                DecodeChannelsOpRet(tx.vout.back().scriptPubKey,tmp_txid,srcpub,destpub,param1,param2,param3) == 'O')
            {
    ...
    {
        if (GetTransaction(channeltxid,tx,hashBlock,false) && !tx.vout.empty() &&
            DecodeChannelsOpRet(tx.vout[numvouts-1].scriptPubKey,opentxid,srcpub,destpub,param1,param2,param3) == 'O')
        {
            ...
            for (auto txid : txids)
            {
                ...
                txhash = txid.first.txhash;
                if (txhash!=prevtxhash && GetTransaction(txhash,tx,hashBlock,false) && !tx.vout.empty() )
                {
                    if (DecodeChannelsOpRet(tx.vout.back().scriptPubKey,tmp_txid,srcpub,destpub,param1,param2,param3) == 'O' && tx.GetHash()==channeltxid)
                    {
                        sprintf(str1,"%lld payments of %lld satoshi",(long long)param1,(long long)param2);
                        result.push_back(Pair("Open", str1));
                    }
                    else if (DecodeChannelsOpRet(tx.vout.back().scriptPubKey,opentxid,srcpub,destpub,param1,param2,param3) == 'P' && opentxid==channeltxid)
                    {
                        if (GetTransaction(opentxid,opentx,hashBlock,false) && !opentx.vout.empty() &&
                            DecodeChannelsOpRet(opentx.vout.back().scriptPubKey,tmp_txid,srcpub,destpub,numpayments,payment,hashchain) == 'O')
                        {
                    ...
                    else if (DecodeChannelsOpRet(tx.vout.back().scriptPubKey,opentxid,srcpub,destpub,param1,param2,param3) == 'C' && opentxid==channeltxid)
                    {
                        result.push_back(Pair("Close","channel"));
                    }
                    else if (DecodeChannelsOpRet(tx.vout.back().scriptPubKey,opentxid,srcpub,destpub,param1,param2,param3) == 'R' && opentxid==channeltxid)
                    {
