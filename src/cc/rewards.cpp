...
uint8_t DecodeRewardsFundingOpRet(const CScript &scriptPubKey,uint64_t &sbits,uint64_t &APR,uint64_t &minseconds,uint64_t &maxseconds,uint64_t &mindeposit)
{
    ...
    if (vopret.size() > 2 && E_UNMARSHAL(vopret,ss >> e; ss >> f; ss >> sbits; ss >> APR; ss >> minseconds; ss >> maxseconds; ss >> mindeposit) != 0 && e == EVAL_REWARDS && f == 'F')
        return f;
    return 0;
}
...
{
    char destaddr[64]; uint64_t sbits; uint256 fundingtxid,txid;
    if (tx.vout[v].scriptPubKey.IsPayToCryptoCondition() && !tx.vout.empty())
    {
        txid = tx.GetHash();
        if ( DecodeRewardsOpRet(txid,tx.vout.back().scriptPubKey,sbits,fundingtxid) != 0 && sbits == refsbits && (fundingtxid == reffundingtxid || txid == reffundingtxid) && Getscriptaddress(destaddr,tx.vout[v].scriptPubKey) && strcmp(destaddr,cp->unspendableCCaddr) == 0)
...
bool RewardsExactAmounts(struct CCcontract_info *cp,Eval *eval,const CTransaction &tx,uint64_t txfee,uint64_t refsbits,uint256 reffundingtxid)
{
    ...
    for (auto txvin : tx.vin)
    {
        if ((*cp->ismyvin)(txvin.scriptSig))
        {
            ...
            inputs += IsRewardsvout(cp,vinTx,tx.vin[i].prevout.n,refsbits,reffundingtxid);
        }
    }
    for (i=0; i<numvouts; i++)
    {
        //fprintf(stderr,"i.%d of numvouts.%d\n",i,numvouts);
        outputs += IsRewardsvout(cp,tx,i,refsbits,reffundingtxid);
    }
...
bool RewardsValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    if ( (funcid=  DecodeRewardsOpRet(txid,tx.vout[numvouts-1].scriptPubKey,sbits,fundingtxid)) != 0 )
    {
        ...
        if ( !fundingTx.vout.empty() && DecodeRewardsFundingOpRet(fundingTx.vout.back().scriptPubKey,sbits,APR,minseconds,maxseconds,mindeposit) != 'F' )
        ...
        switch ( funcid )
        {
            ...
            case 'U':
                ...
                if ( DecodeRewardsOpRet(tx.vin[0].prevout.hash,vinTx.vout.back().scriptPubKey,vinsbits,vinfundingtxid) != 'L' )
                    return eval->Invalid("can only unlock locktxid");
                if ( fundingtxid != vinfundingtxid )
                ...
                if ( numvouts == 2 && numvins == 1 )
                {
                    ...
                    if ( tx.vout[0].scriptPubKey != vinTx.vout[1].scriptPubKey )
                        return eval->Invalid("unlock recover tx vout.0 mismatched scriptPubKey");
                    if ( tx.vout[0].nValue > vinTx.vout[0].nValue )
                ...
                if (!vinTx.vout[0].scriptPubKey.IsPayToCryptoCondition())
                ...
                if ( tx.vout[1].scriptPubKey != vinTx.vout[1].scriptPubKey )
                ...
                if (!RewardsExactAmounts(cp,eval,tx,txfee+tx.vout[1].nValue,sbits,fundingtxid))
                    return false;
                if ( tx.vout[1].nValue > amount+reward )
                    return eval->Invalid("unlock tx vout.1 isnt amount+reward");
                if ( tx.vout[2].nValue > 0 )
...
static uint64_t myIs_unlockedtx_inmempool(uint256 &txid,int32_t &vout,uint64_t refsbits,uint256 reffundingtxid,uint64_t needed)
{
    ...
    for (const CTxMemPoolEntry &e : mempool.mapTx)
    {
        ...
        if (!tx.vout.empty() && tx.vout[0].nValue >= needed)
        {
            const uint256 &hash = tx.GetHash();
            if (tx.vout[0].scriptPubKey.IsPayToCryptoCondition() && !myIsutxo_spentinmempool(hash,0))
            {
                if ((funcid= DecodeRewardsOpRet(hash,tx.vout.back().scriptPubKey,sbits,fundingtxid)) == 'U' && sbits == refsbits && fundingtxid == reffundingtxid)
                {
                    txid = hash;
                    vout = 0;
...
int64_t AddRewardsInputs(CScript &scriptPubKey,uint64_t maxseconds,struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey pk,int64_t total,int32_t maxinputs,uint64_t refsbits,uint256 reffundingtxid)
{
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        if ( j != mtx.vin.size() )
            continue;
        if (GetTransaction(txid,tx,hashBlock,false) && !tx.vout.empty() && tx.vout[vout].scriptPubKey.IsPayToCryptoCondition() && !myIsutxo_spentinmempool(txid,vout))
        {
            if ((funcid= DecodeRewardsOpRet(txid,tx.vout.back().scriptPubKey,sbits,fundingtxid)))
            {
                if ( sbits != refsbits || fundingtxid != reffundingtxid )
                    continue;
                if ( maxseconds == 0 && funcid != 'F' && funcid != 'A' && funcid != 'U' )
                    continue;
                if ( maxseconds != 0 && funcid != 'L' && CCduration(numblocks,txid) < maxseconds )
                    continue;
                fprintf(stderr,"maxseconds.%d (%c) %.8f %.8f\n",(int32_t)maxseconds,funcid,(double)tx.vout[vout].nValue/COIN,(double)unspentOutput.second.satoshis/COIN);
...
int64_t RewardsPlanFunds(uint64_t &lockedfunds,uint64_t refsbits,struct CCcontract_info *cp,CPubKey pk,uint256 reffundingtxid)
{
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        txid = unspentOutput.first.txhash;
        vout = (int32_t)unspentOutput.first.index;
        if (GetTransaction(txid,tx,hashBlock,false) && tx.vout[vout].scriptPubKey.IsPayToCryptoCondition())
        {
            if ((funcid= DecodeRewardsOpRet(txid,tx.vout.back().scriptPubKey,sbits,fundingtxid)) == 'F' || funcid == 'A' || funcid == 'U' || funcid == 'L')
            {
...
bool RewardsPlanExists(struct CCcontract_info *cp,uint64_t refsbits,CPubKey rewardspk,uint64_t &APR,uint64_t &minseconds,uint64_t &maxseconds,uint64_t &mindeposit)
{
    ...
    for (auto it : txids)
    {
        ...
        if (GetTransaction(txid,tx,hashBlock,false) && !tx.vout.empty() && ConstrainVout(tx.vout[0],1,CCaddr,0))
        {
            //char str[65]; fprintf(stderr,"rewards plan %s\n",uint256_str(str,txid));
            if (DecodeRewardsFundingOpRet(tx.vout.back().scriptPubKey,sbits,APR,minseconds,maxseconds,mindeposit) == 'F' && sbits == refsbits)
                return true;
        }
    }
    return false;
...
UniValue RewardsInfo(uint256 rewardsid)
{
    ...
    if (!GetTransaction(rewardsid,vintx,hashBlock,false))
    {
    ...
    if (!vintx.vout.empty() && DecodeRewardsFundingOpRet(vintx.vout.back().scriptPubKey,sbits,APR,minseconds,maxseconds,mindeposit) == 0)
    {
    ...
    rewardspk = GetUnspendable(cp, nullptr);
...
UniValue RewardsList()
{
    ...
    for (auto address : addressIndex)
    {
        ...
        if (GetTransaction(txid,vintx,hashBlock,false) && !vintx.vout.empty() && DecodeRewardsFundingOpRet(vintx.vout.back().scriptPubKey,sbits,APR,minseconds,maxseconds,mindeposit) != 0 )
...
std::string RewardsCreateFunding(uint64_t txfee,char *planstr,int64_t funds,int64_t APR,int64_t minseconds,int64_t maxseconds,int64_t mindeposit)
{
    ...
    if (RewardsPlanExists(cp,sbits,rewardspk,a,b,c,d))
    {
...
std::string RewardsLock(uint64_t txfee,char *planstr,uint256 fundingtxid,int64_t deposit)
{
    ...
    rewardspk = GetUnspendable(cp, nullptr);
    sbits = stringbits(planstr);
    if (!RewardsPlanExists(cp,sbits,rewardspk,APR,minseconds,maxseconds,mindeposit))
    {
        CCerror = strprintf("Rewards plan %s doesnt exist\n",planstr);
        fprintf(stderr,"%s\n",CCerror.c_str());
        return "";
...
std::string RewardsUnlock(uint64_t txfee,char *planstr,uint256 fundingtxid,uint256 locktxid)
{
    ...
    else
    {
        ...
        if (!GetTransaction(locktxid,tx,hashBlock,false) || tx.vout.empty() || tx.vout[1].scriptPubKey.IsPayToCryptoCondition())
        {
