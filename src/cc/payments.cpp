...
bool PaymentsExactAmounts(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx,int32_t minage,uint64_t txfee)
{
    ...
    for (auto txvin : tx.vin)
    {
        ...
        if ((*cp->ismyvin)(txvin.scriptSig))
        {
            ...
            inputs += IsPaymentsvout(cp,vinTx,txvin.prevout.n);
        }
    }
    for (i=0; i<numvouts; i++)
    {
        //fprintf(stderr,"i.%d of numvouts.%d\n",i,numvouts);
        outputs += IsPaymentsvout(cp,tx,i);
    }
...
bool PaymentsValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    for (i=0; i<numvins; i++) //TODO: Useless loop
        if (!IsCCInput(tx.vin[0].scriptSig))
            return eval->Invalid("illegal normal vini");
    //fprintf(stderr,"check amounts\n");
    if (!PaymentsExactAmounts(cp,eval,tx,1,10000))
    {
    ...
    if (retval)
        ...
        fprintf(stderr,"Paymentsget invalid\n");
    return retval;
}
...
int64_t AddPaymentsInputs(struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey pk,int64_t total,int32_t maxinputs)
{
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        if (GetTransaction(txid,vintx,hashBlock,false) && (nValue= IsPaymentsvout(cp,vintx,vout)) > 1000000 && !myIsutxo_spentinmempool(txid,vout))
        {
...
std::string PaymentsGet(uint64_t txfee,int64_t nValue)
{
    ...
    if ( txfee == 0 )
        txfee = 10000;
    Paymentspk = GetUnspendable(cp, nullptr);
    ...
    fprintf(stderr,"cant find Payments inputs\n");
    return "";
}
