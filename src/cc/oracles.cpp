...
uint8_t DecodeOraclesCreateOpRet(const CScript &scriptPubKey,std::string &name,std::string &description,std::string &format)
{
    ...
    if ( script[0] == EVAL_ORACLES && script[1] == 'C' ) {
...
{
    uint256 txid,oracletxid,hashBlock; int64_t datafee=0,dfee; int32_t dheight=0,vout,height; CTransaction tx; CPubKey pk;
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        height = (int32_t)unspentOutput.second.blockHeight;
        if (myGetTransaction(txid,tx,hashBlock) && !tx.vout.empty())
        {
...
{
    CTransaction oracletx; char markeraddr[64]; uint256 hashBlock; std::string name,description,format;
    if (myGetTransaction(oracletxid,oracletx,hashBlock) && !oracletx.vout.empty())
    {
        if (DecodeOraclesCreateOpRet(oracletx.vout.back().scriptPubKey,name,description,format) == 'C')
        {
            CCtxidaddr(markeraddr,oracletxid);
            return OracleCurrentDatafee(oracletxid,markeraddr,publisher);
        }
...
static uint256 myIs_baton_spentinmempool(uint256 batontxid,int32_t batonvout)
{
    for (const CTxMemPoolEntry &e : mempool.mapTx) {
        ...
        if (!tx.vout.empty() && tx.vin.size() > 1 && batontxid == tx.vin[1].prevout.hash && batonvout == tx.vin[1].prevout.n )
        {
...
{
    uint256 txid,oracletxid,hashBlock,btxid,batontxid = zeroid; int64_t dfee; int32_t dheight=0,vout,height; CTransaction tx; CPubKey pk; uint8_t *ptr; std::vector<uint8_t> vopret,data;
    std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > unspentOutputs;
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        vout = (int32_t)unspentOutput.first.index;
        height = (int32_t)unspentOutput.second.blockHeight;
        if (unspentOutput.second.satoshis != txfee )
        {
            fprintf(stderr,"it->second.satoshis %llu != %llu txfee\n",(long long)unspentOutput.second.satoshis,(long long)txfee);
            continue;
        }
        if (myGetTransaction(txid,tx,hashBlock) && !tx.vout.empty())
        {
            ...
            if ( vopret.size() > 2 )
            {
                ...
                if ( (ptr[1] == 'D' && DecodeOraclesData(tx.vout.back().scriptPubKey,oracletxid,btxid,pk,data) == 'D') ||
                     (ptr[1] == 'R' && DecodeOraclesOpRet(tx.vout.back().scriptPubKey,oracletxid,pk,dfee) == 'R') )
                {
                    if ( oracletxid == reforacletxid && pk == publisher && height > dheight )
                    {
                        dheight = height;
                        batontxid = txid;
                        if ( ptr[1] == 'D' )
    ...
    while (myIsutxo_spentinmempool(batontxid,1))
...
{
    std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > unspentOutputs;
    CTransaction regtx; uint256 hash,txid,oracletxid; CPubKey pk; int32_t height,maxheight=0; int64_t datafee; char markeraddr[64],batonaddr[64]; std::vector <uint8_t> data; struct CCcontract_info *cp,C;
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        {
            if (!regtx.vout.empty() && DecodeOraclesOpRet(regtx.vout.back().scriptPubKey,oracletxid,pk,datafee) == 'R' && oracletxid == reforacletxid && pk == refpk)
            {
                Getscriptaddress(batonaddr,regtx.vout[1].scriptPubKey);
                return OracleBatonUtxo(10000,cp,oracletxid,batonaddr,pk,data);
            }
        }
    }
    return zeroid;
...
{
    std::vector <int64_t> sorted; int32_t i; int64_t *prices,price;
    if (origprices.size() == 1)
        return origprices[0];
    std::sort(origprices.begin(), origprices.end());
    prices = (int64_t *)calloc(origprices.size(),sizeof(*prices));
    i = 0;
    for (auto s : sorted)
        prices[i++] = s;
...
int64_t OraclePrice(int32_t height,uint256 reforacletxid,char *markeraddr,char *format)
{
    ...
    {
        txid = unspentOutput.first.txhash;
        ht = (int32_t)unspentOutput.second.blockHeight;
        if (myGetTransaction(txid,regtx,hash))
        {
            if (!regtx.vout.empty() && DecodeOraclesOpRet(regtx.vout.back().scriptPubKey,oracletxid,pk,datafee) == 'R' && oracletxid == reforacletxid)
            {
                Getscriptaddress(batonaddr,regtx.vout[1].scriptPubKey);
    ...
    if ( maxheight > 10 )
    {
        ...
        {
            if (publisher.height >= maxheight-10)
            {
                oracle_format(&hash,&price,nullptr,'L',(uint8_t *)publisher.data.data(),0,(int32_t)publisher.data.size());
                if ( price != 0 )
...
{
    //char destaddr[64];
    return tx.vout[v].scriptPubKey.IsPayToCryptoCondition() ? tx.vout[v].nValue : 0;
    ...
    for (i=0; i<numvins; i++)
    {
        ...
        if ((*cp->ismyvin)(tx.vin[i].scriptSig))
        {
            ...
            if ( (assetoshis= IsOraclesvout(cp,vinTx,tx.vin[i].prevout.n)) != 0 )
            {
                ...
                if ( i != 1 && scriptPubKey == vinTx.vout[tx.vin[i].prevout.n].scriptPubKey )
                    inputs += assetoshis;
            }
        }
    }
    ...
    if ( vopret.size() > 2 )
    {
        ...
        switch ( script[1] )
        {
            ...
            case 'D': // data
                ...
                // vout.2: payment for dataprovider
                // vout.3: change, if any
                if ( numvins >= 2 && numvouts >= 3 && DecodeOraclesData(tx.vout.back().scriptPubKey,oracletxid,batontxid,publisher,data) == 'D' )
                    return OraclesDataValidate(cp,eval,tx,oracletxid,publisher,tx.vout[2].nValue);
...
{
    char coinaddr[64],funcid; int64_t nValue,price,totalinputs = 0; uint256 tmporacletxid,tmpbatontxid,txid,hashBlock; std::vector<uint8_t> origpubkey,data; CTransaction vintx; int32_t vout,n = 0;
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        if (GetTransaction(txid,vintx,hashBlock,false) && vintx.vout.size() > 1)
        {
            if ((funcid=DecodeOraclesOpRet(vintx.vout.back().scriptPubKey,tmporacletxid,tmppk,tmpnum))!=0 && (funcid=='S' || funcid=='D'))
            {
                if (funcid=='D' && DecodeOraclesData(vintx.vout.back().scriptPubKey,tmporacletxid,tmpbatontxid,tmppk,data)==0)
                    fprintf(stderr,"invalid oraclesdata transaction \n");
                else if (tmporacletxid==oracletxid)
                {
                    // get valid CC payments
                    if ( (nValue= IsOraclesvout(cp,vintx,vout)) >= 10000 && !myIsutxo_spentinmempool(txid,vout))
                    {
...
int64_t LifetimeOraclesFunds(struct CCcontract_info *cp,uint256 oracletxid,CPubKey publisher)
{
    ...
    for (auto address : addressIndex)
    {
        ...
        if (GetTransaction(txid,subtx,hashBlock,false) &&!subtx.vout.empty() && DecodeOraclesOpRet(subtx.vout.back().scriptPubKey,subtxid,pk,num) == 'S' && subtxid == oracletxid && pk == publisher )
...
std::string OracleCreate(int64_t txfee,std::string name,std::string description,std::string format)
{
    ...
    Oraclespk = GetUnspendable(cp, nullptr);
...
UniValue OracleDataSamples(uint256 reforacletxid,uint256 batontxid,int32_t num)
{
    ...
    if (GetTransaction(reforacletxid,oracletx,hashBlock,false) && !oracletx.vout.empty() && DecodeOraclesCreateOpRet(oracletx.vout.back().scriptPubKey,name,description,format) == 'C')
    {
        while (GetTransaction(batontxid,tx,hashBlock,false) && !tx.vout.empty() && DecodeOraclesData(tx.vout.back().scriptPubKey,oracletxid,btxid,pk,data) == 'D' && reforacletxid == oracletxid)
        {
...
UniValue OracleInfo(uint256 origtxid)
{
    ...
    if (!GetTransaction(origtxid,tx,hashBlock,false))
    {
    ...
    if (!tx.vout.empty() && DecodeOraclesCreateOpRet(tx.vout.back().scriptPubKey,name,description,format) == 'C')
    {
        result.push_back(Pair("result","success"));
        ...
        {
            txid = unspentOutput.first.txhash;
            if (GetTransaction(txid,regtx,hashBlock,false) && !regtx.vout.empty() && DecodeOraclesOpRet(regtx.vout.back().scriptPubKey,oracletxid,pk,datafee) == 'R' && oracletxid == origtxid )
            {
...
UniValue OraclesList()
{
    ...
    {
        txid = address.first.txhash;
        if (GetTransaction(txid,createtx,hashBlock,false) && !createtx.vout.empty() && DecodeOraclesCreateOpRet(createtx.vout.back().scriptPubKey,name,description,format) == 'C')
