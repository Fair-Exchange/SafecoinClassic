...
bool FSMExactAmounts(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx,int32_t minage,uint64_t txfee)
{
    ...
    for (auto txvin : tx.vin)
    {
        ...
        if ((*cp->ismyvin)(txvin.scriptSig))
        {
            ...
            inputs += IsFSMvout(cp,vinTx,txvin.prevout.n);
        }
    }
    for (i=0; i<numvouts; i++)
    {
        //fprintf(stderr,"i.%d of numvouts.%d\n",i,numvouts);
        outputs += IsFSMvout(cp,tx,i);
    }
...
bool FSMValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    int32_t numvins,preventCCvins,preventCCvouts,i; bool retval;
    numvins = tx.vin.size();
    ...
    if (!FSMExactAmounts(cp,eval,tx,1,10000))
    {
        fprintf(stderr,"fsmget invalid amount\n");
        return false;
    }
    i = IsFSMvout(cp,tx,0) != 0 ? 1 : 0;
    preventCCvouts = 1 + i;
...
int64_t AddFSMInputs(struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey pk,int64_t total,int32_t maxinputs)
{
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        // prevent dup
        if ( unspentOutput.second.satoshis < 1000000 )
            continue;
        if (GetTransaction(txid,vintx,hashBlock,false))
        {
            if ( (nValue= IsFSMvout(cp,vintx,(int32_t)unspentOutput.first.index)) > 0 )
            {
...
std::string FSMCreate(uint64_t txfee,std::string name,std::string states)
{
    ...
    if ( txfee == 0 )
        txfee = 10000;
    fsmpk = GetUnspendable(cp, nullptr);
    ...
    fprintf(stderr,"cant find fsm inputs\n");
    return "";
}
