...
bool LottoExactAmounts(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx,int32_t minage,uint64_t txfee)
{
    ...
    CTransaction vinTx; uint256 hashBlock,activehash; int32_t i,numvouts; int64_t inputs=0,outputs=0,assetoshis;
    ...
    for (auto txvin : tx.vin)
    {
        ...
        if ((*cp->ismyvin)(txvin.scriptSig))
        {
            ...
            inputs += IsLottovout(cp,vinTx,tx.vin[i].prevout.n);
        }
    }
    for (i=0; i<numvouts; i++)
    {
        //fprintf(stderr,"i.%d of numvouts.%d\n",i,numvouts);
        outputs += IsLottovout(cp,tx,i);
    }
...
bool LottoValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    int32_t preventCCvins,preventCCvouts,i; bool retval;
    ...
    if (!LottoExactAmounts(cp,eval,tx,1,10000))
    {
    ...
    if (retval)
        ...
        fprintf(stderr,"Lottoget invalid\n");
    return retval;
}
...
int64_t AddLottoInputs(struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey pk,int64_t total,int32_t maxinputs)
{
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        // prevent dup
        if ( unspentOutput.second.satoshis < COIN )
            continue;
        if (GetTransaction(txid,vintx,hashBlock,false))
        {
            if ( (nValue= IsLottovout(cp,vintx,(int32_t)unspentOutput.first.index)) > 0 )
            {
                if ( total != 0 && maxinputs != 0 )
                    mtx.vin.push_back(CTxIn(txid,(int32_t)unspentOutput.first.index,CScript()));
...
uint8_t DecodeLottoFundingOpRet(const CScript &scriptPubKey,uint64_t &sbits,int32_t ticketsize,int32_t odds,int32_t firstheight,int32_t period,uint256 hentropy)
{
    ...
    if (vopret.size() > 2 && E_UNMARSHAL(vopret,ss >> e; ss >> f; ss >> sbits; ss >> ticketsize; ss >> odds; ss >> firstheight; ss >> period; ss >> hentropy) != 0 && e == EVAL_LOTTO && f == 'F')
        return f;
    return 0;
}
...
UniValue LottoInfo(uint256 lottoid)
{
    ...
    if (!GetTransaction(lottoid,vintx,hashBlock,false))
    {
    ...
    if ( !vintx.vout.empty() && DecodeLottoFundingOpRet(vintx.vout.back().scriptPubKey,sbits,ticketsize,odds,firstheight,period,hentropy) == 0 )
    {
    ...
    lottopk = GetUnspendable(cp, nullptr);
...
UniValue LottoList()
{
    ...
    for (auto address : addressIndex)
    {
        ...
        if (GetTransaction(txid,vintx,hashBlock,false) && !vintx.vout.empty() && DecodeLottoFundingOpRet(vintx.vout.back().scriptPubKey,sbits,ticketsize,odds,firstheight,period,hentropy) == 'F' )
...
std::string LottoTicket(uint64_t txfee,uint256 lottoid,int64_t numtickets)
{
    ...
    fprintf(stderr,"cant find Lotto inputs\n");
    return "";
}
