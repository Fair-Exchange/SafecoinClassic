...
uint8_t DecodeGatewaysBindOpRet(char *depositaddr,const CScript &scriptPubKey,std::string &coin,uint256 &tokenid,int64_t &totalsupply,uint256 &oracletxid,uint8_t &M,uint8_t &N,std::vector<CPubKey> &pubkeys,uint8_t &taddr,uint8_t &prefix,uint8_t &prefix2)
{
    ...
    if ( vopret.size() > 2 && E_UNMARSHAL(vopret,ss >> e; ss >> f; ss >> coin; ss >> prefix; ss >> prefix2; ss >> taddr; ss >> tokenid; ss >> totalsupply; ss >> M; ss >> N; ss >> pubkeys; ss >> oracletxid) != 0 )
    {
        ...
        else fprintf(stderr,"need to generate non-SAFE addresses prefix.%d\n",prefix);
        return f;
    }
    fprintf(stderr,"error decoding bind opret\n");
    return 0;
}
...
uint8_t DecodeGatewaysOpRet(const CScript &scriptPubKey)
{
    ...
    if ( vopret.size() > 2 && (script[0] == EVAL_GATEWAYS || script[0] == EVAL_ASSETS) && E_UNMARSHAL(vopret,ss >> e; ss >> f) != 0 )
        if (f == 'B' && f == 'D' && f == 't' && f == 'W' && f == 'P' && f == 'M') // TODO: I don't think it's gonna happen...
          return f;
    return 0;
}
...
bool GatewaysExactAmounts(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx,int32_t minage,uint64_t txfee)
{
    ...
    for (auto in : tx.vin)
    {
        ...
        if ( (*cp->ismyvin)(in.scriptSig) != 0 )
        {
            ...
            inputs += IsGatewaysvout(cp,vinTx,in.prevout.n);
        }
    }
    for (i=0; i<numvouts; i++)
    {
        //fprintf(stderr,"i.%d of numvouts.%d\n",i,numvouts);
        outputs += IsGatewaysvout(cp,tx,i);
    }
...
static bool myIs_coinaddr_inmempoolvout(char *coinaddr)
{
    char destaddr[64];
    for (const CTxMemPoolEntry &e : mempool.mapTx) {
        ...
        if (!tx.vout.empty())
        {
            ...
            {
                Getscriptaddress(destaddr, out.scriptPubKey);
...
{
    CTransaction tx; uint256 hash,mhash,bhash,hashBlock,oracletxid; int32_t len,len2; int64_t val,merkleht; CPubKey pk; std::vector<uint8_t>data;
    txid = zeroid;
    char str[65];
    //fprintf(stderr,"start reverse scan %s\n",uint256_str(str,batontxid));
    while (myGetTransaction(batontxid,tx,hashBlock) && !tx.vout.empty())
    {
        //fprintf(stderr,"check %s\n",uint256_str(str,batontxid));
        if ( DecodeOraclesData(tx.vout.back().scriptPubKey,oracletxid,bhash,pk,data) != 'D' || oracletxid != reforacletxid )
            break;
        //fprintf(stderr,"decoded %s\n",uint256_str(str,batontxid));
        if ( oracle_format(&hash,&merkleht,nullptr,'I',(uint8_t *)data.data(),0,(int32_t)data.size()) == sizeof(int32_t) && merkleht == height )
        {
            len = oracle_format(&hash,&val,nullptr,'h',(uint8_t *)data.data(),sizeof(int32_t),(int32_t)data.size());
            len2 = oracle_format(&mhash,&val,nullptr,'h',(uint8_t *)data.data(),(int32_t)(sizeof(int32_t)+sizeof(uint256)),(int32_t)data.size());
...
int8_t GatewaysCointxidExists(struct CCcontract_info *cp,uint256 cointxid) // dont forget to check mempool!
{
    ...
    if (!addressIndex.empty())
        return -1;
    return myIs_coinaddr_inmempoolvout(txidaddr) ? 1 : 0;
}
...
{
    std::vector<uint256> txids; uint256 proofroot,hashBlock,txid = zeroid; CTransaction tx; std::string name,description,format; char destaddr[64],destpubaddr[64],claimaddr[64],str[65],str2[65]; int32_t i; int64_t nValue = 0;
    if (!myGetTransaction(oracletxid,tx,hashBlock) || tx.vout.empty())
    {
        fprintf(stderr,"GatewaysVerify cant find oracletxid %s\n",uint256_str(str,oracletxid));
        return 0;
    }
    if ( DecodeOraclesCreateOpRet(tx.vout.back().scriptPubKey,name,description,format) != 'C' || name != refcoin )
    {
    ...
    if (DecodeHexTx(tx,deposithex))
    {
        ...
        {
            for (auto txvout : tx.vout)
            {
                Getscriptaddress(destaddr,txvout.scriptPubKey);
                if ( strcmp(refdepositaddr,destaddr) == 0 )
                {
                    txid = tx.GetHash();
                    nValue = txvout.nValue;
                    break;
                }
            }
...
{
    int32_t claimvout,height; int64_t amount; std::string coin,deposithex; std::vector<CPubKey> publishers; std::vector<uint256>txids; uint256 bindtxid,cointxid; std::vector<uint8_t> proof; CPubKey claimpubkey;
    if (!tx.vout.empty() && DecodeGatewaysDepositOpRet(tx.vout.back().scriptPubKey,coin,bindtxid,publishers,txids,height,cointxid,claimvout,deposithex,proof,claimpubkey,amount) == 'D' && claimpubkey == mypk ) {
...
bool GatewaysValidate(struct CCcontract_info *cp,Eval *eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    // {
        ...
        if ( (funcid = DecodeGatewaysOpRet(tx.vout.back().scriptPubKey)) != 0)
        {
            switch ( funcid )
            {
                case 'B':
                ...
                case 't':
                    ...
                    if (!tx.vout.empty() && DecodeGatewaysClaimOpRet(tx.vout.back().scriptPubKey,assetid,refcoin,bindtxid,deposittxid,destpub,amount)==0)
                        return eval->Invalid("invalid gatewaysclaim OP_RETURN data!");
                    if ( IsCCInput(tx.vin[0].scriptSig) != 0 )
                        return eval->Invalid("vin.0 is normal for gatewaysClaim!");
                    if ( IsCCInput(tx.vin[1].scriptSig) == 0 )
                        return eval->Invalid("vin.1 is CC for gatewaysClaim!");
                    if ( IsCCInput(tx.vin[2].scriptSig) == 0 )
                    ...
                    if (numvouts > 2 && !tx.vout[1].scriptPubKey.IsPayToCryptoCondition())
                        return eval->Invalid("vout.1 is CC for gatewaysClaim!");
                    if (!myGetTransaction(bindtxid,bindtx,hashblock))
                        return eval->Invalid("invalid gatewaysbind txid!");
                    if (!bindtx.vout.empty() && DecodeGatewaysBindOpRet(depositaddr,bindtx.vout.back().scriptPubKey,tmprefcoin,tokenid,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2) != 'B')
                    ...
                    if (!safecoin_txnotarizedconfirmed(bindtxid))
                        return eval->Invalid("gatewaysbind tx is not yet confirmed(notarised)!");
                    if (!myGetTransaction(deposittxid,deposittx,hashblock))
                        return eval->Invalid("invalid gatewaysdeposittxid!");
                    if (!deposittx.vout.empty() && DecodeGatewaysDepositOpRet(deposittx.vout.back().scriptPubKey,tmprefcoin,tmptxid,tmppublishers,txids,height,cointxid,claimvout,deposithex,proof,tmpdestpub,tmpamount) != 'D')
                    ...
                    if (tmpamount>totalsupply)
                        return eval->Invalid("deposit amount greater then bind total supply");
                    if (!safecoin_txnotarizedconfirmed(deposittxid))
        ...
        if (retval)
            ...
            fprintf(stderr,"Gatewaysget invalid\n");
        return retval;
        // }
}
...
bool GatewaysBindExists(struct CCcontract_info *cp,CPubKey gatewayspk,uint256 reftokenid) // dont forget to check mempool!
{
    ...
    {
        if (GetTransaction(address.first.txhash,tx,hashBlock,false) && !tx.vout.empty())
        {
            if ( DecodeGatewaysBindOpRet(depositaddr,tx.vout.back().scriptPubKey,coin,tokenid,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2) == 'B' )
            {
...
std::string GatewaysBind(uint64_t txfee,std::string coin,uint256 tokenid,int64_t totalsupply,uint256 oracletxid,uint8_t M,uint8_t N,std::vector<CPubKey> pubkeys)
{
    ...
    gatewayspk = GetUnspendable(cp, nullptr);
...
std::string GatewaysDeposit(uint64_t txfee,uint256 bindtxid,int32_t height,std::string refcoin,uint256 cointxid,int32_t claimvout,std::string deposithex,std::vector<uint8_t>proof,CPubKey destpub,int64_t amount)
{
    ...
    int64_t totalsupply; int32_t i,m; uint8_t M,N,taddr,prefix,prefix2; std::string coin; struct CCcontract_info *cp,C;
    ...
    if (!GetTransaction(bindtxid,bindtx,hashBlock,false) || !bindtx.vout.empty())
    {
        fprintf(stderr,"cant find bindtxid %s\n",uint256_str(str,bindtxid));
        return "";
    }
    if ( DecodeGatewaysBindOpRet(depositaddr,bindtx.vout.back().scriptPubKey,coin,tokenid,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2) != 'B' || refcoin != coin )
    {
    ...
    {
        fprintf(stderr,"pubkeys[%d] %s\n",i,pubkey33_str(str,(uint8_t *)&pubkey));
        if ( (mhash= GatewaysReverseScan(txid,height,oracletxid,OraclesBatontxid(oracletxid,pubkey))) != zeroid )
        {
            if ( merkleroot == zeroid )
                merkleroot = mhash, m = 1;
            else if ( mhash == merkleroot )
                m++;
            publishers.push_back(pubkey);
            txids.push_back(txid);
        }
    }
    fprintf(stderr,"cointxid.%s m.%d of n.%d\n",uint256_str(str,cointxid),m,pubkeys.size());
    if ( merkleroot == zeroid || m < pubkeys.size()/2 )
    {
        //uint256 tmp;
        //decode_hex((uint8_t *)&tmp,32,(char *)"90aedc2f19200afc9aca2e351438d011ebae8264a58469bf225883045f61917f");
        //merkleroot = revuint256(tmp);
        fprintf(stderr,"couldnt find merkleroot for ht.%d %s oracle.%s m.%d vs n.%d\n",height,coin.c_str(),uint256_str(str,oracletxid),m,pubkeys.size());
        return "";
    }
    if ( GatewaysCointxidExists(cp,cointxid) != 0 )
    {
...
std::string GatewaysClaim(uint64_t txfee,uint256 bindtxid,std::string refcoin,uint256 deposittxid,CPubKey destpub,int64_t amount)
{
    ...
    int32_t claimvout,height; std::vector<uint8_t> proof;
    ...
    mypk = pubkey2pk(Mypubkey());
    gatewayspk = GetUnspendable(cp, nullptr);
    if (!GetTransaction(bindtxid,tx,hashBlock,false) || tx.vout.empty())
    {
        fprintf(stderr,"cant find bindtxid %s\n",uint256_str(str,bindtxid));
        return "";
    }
    if ( DecodeGatewaysBindOpRet(depositaddr,tx.vout.back().scriptPubKey,coin,assetid,totalsupply,oracletxid,M,N,msigpubkeys,taddr,prefix,prefix2) != 'B' || coin != refcoin )
    {
        fprintf(stderr,"invalid coin - bindtxid %s coin.%s\n",uint256_str(str,bindtxid),coin.c_str());
        return "";
    }
    if (!GetTransaction(deposittxid,tx,hashBlock,false) || tx.vout.empty())
    {
        fprintf(stderr,"cant find deposittxid %s\n",uint256_str(str,bindtxid));
        return "";
    }
    if (DecodeGatewaysDepositOpRet(tx.vout.back().scriptPubKey,coin,tmptxid,publishers,txids,height,cointxid,claimvout,deposithex,proof,tmpdestpub,tmpamount) != 'D' || coin != refcoin)
    {
...
std::string GatewaysPartialSign(uint64_t txfee,uint256 txid,std::string refcoin, std::string hex)
{
    ...
    else
    {
        for (auto unspentOutput : unspentOutputs)
        {
            tmptxid = unspentOutput.first.txhash;
            if (GetTransaction(tmptxid,tx,hashBlock,false)  && !tx.vout.empty() && DecodeGatewaysPartialOpRet(tx.vout.back().scriptPubKey,K,signerpk,refcoin,hex) == 'P' && K>maxK )
            {
                maxK=K;
                parttxid=tmptxid;
            }
...
UniValue GatewaysPendingWithdraws(uint256 bindtxid,std::string refcoin)
{
    ...
    int32_t i,vout,queueflag; int64_t totalsupply,amount,nValue; struct CCcontract_info *cp,C;
    std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > unspentOutputs;
    ...
    if ( DecodeGatewaysBindOpRet(depositaddr,tx.vout.back().scriptPubKey,tmprefcoin,assetid,totalsupply,oracletxid,M,N,msigpubkeys,taddr,prefix,prefix2) != 'B' || tmprefcoin != refcoin )
    {
        fprintf(stderr,"invalid bindtxid %s coin.%s\n",uint256_str(str,bindtxid),tmprefcoin.c_str());
        return result;
    }
    queueflag = 0;
    for (auto msigpubkey : msigpubkeys)
        if ( msigpubkey == mypk )
        {
            queueflag = 1;
            break;
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        txid = unspentOutput.first.txhash;
        vout = (int32_t)unspentOutput.first.index;
        nValue = (int64_t)unspentOutput.second.satoshis;
        fprintf(stderr,"%s %d %ld\n",txid.ToString().c_str(),vout,(long)nValue);
        if ( vout == 2 && nValue == 10000 && GetTransaction(txid,tx,hashBlock,false) && !tx.vout.empty() &&
            DecodeGatewaysWithdrawOpRet(tx.vout.back().scriptPubKey,assetid,tmprefcoin,withdrawpub,amount) == 'W' && !myIsutxo_spentinmempool(txid,vout))
        {
...
UniValue GatewaysProcessedWithdraws(uint256 bindtxid,std::string refcoin)
{
    ...
    int32_t i,vout,queueflag; int64_t totalsupply,nValue,amount; struct CCcontract_info *cp,C;
    std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > unspentOutputs;
    ...
    for (auto unspentOutput : unspentOutputs)
    {
        ...
        nValue = (int64_t)unspentOutput.second.satoshis;
        //fprintf(stderr,"%s %d %ld\n",txid.ToString().c_str(),vout,(long)nValue);
        if (vout == 0 && nValue == 10000 && GetTransaction(txid,tx,hashBlock,false) && !tx.vout.empty() &&
            DecodeGatewaysCompleteSigningOpRet(tx.vout.back().scriptPubKey,tmprefcoin,withdrawtxid,hex) == 'C' && !myIsutxo_spentinmempool(txid,vout) &&
            GetTransaction(withdrawtxid,tx,hashBlock,false) && !tx.vout.empty() && DecodeGatewaysWithdrawOpRet(tx.vout.back().scriptPubKey,assetid,tmprefcoin,withdrawpub,amount) == 'W')
        {
            UniValue obj(UniValue::VOBJ);
            obj.push_back(Pair("txid",uint256_str(str,txid)));
...
UniValue GatewaysMultisig(char *cctxidaddr)
{
    ...
    {
        txid = unspentOutput.first.txhash;
        if (GetTransaction(txid,tx,hashBlock,false) && !tx.vout.empty() && DecodeGatewaysPartialOpRet(tx.vout.back().scriptPubKey,K,signerpk,refcoin,hex) == 'P' && K>maxK )
        {
            maxK=K;
            parthex=hex;
        }
    ...
    for (const CTxMemPoolEntry &e : mempool.mapTx)
    {
        ...
        if (!txmempool.vout.empty() && DecodeGatewaysPartialOpRet(txmempool.vout.back().scriptPubKey,K,signerpk,refcoin,hex) == 'P' && K>maxK)
        {
            maxK=K;
            parthex=hex;
        }
...
UniValue GatewaysList()
{
    ...
    for (auto address : addressIndex)
    {
        ...
        if (GetTransaction(txid,vintx,hashBlock,false) && !vintx.vout.empty() && DecodeGatewaysBindOpRet(depositaddr,vintx.vout.back().scriptPubKey,coin,tokenid,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2) != 0)
...
UniValue GatewaysInfo(uint256 bindtxid)
{
    ...
    if (GetTransaction(bindtxid,tx,hashBlock,false))
    {
        depositaddr[0] = '\0';
        if ( !tx.vout.empty() && DecodeGatewaysBindOpRet(depositaddr,tx.vout.back().scriptPubKey,coin,tokenid,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2) != 0 && M <= N && N > 0 )
        {
            if ( N > 1 )
            {
