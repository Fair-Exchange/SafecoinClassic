...
bool AssetsValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    if ((funcid= DecodeAssetOpRet(tx.vout.back().scriptPubKey,assetid,assetid2,remaining_price,origpubkey)) == 0)
        return eval->Invalid("Invalid opreturn payload");
    fprintf(stderr,"AssetValidate (%c)\n",funcid);
    if (funcid != 'o' && funcid != 'x') {
        if (!eval->GetTxUnconfirmed(assetid,createTx,hashBlock))
            return eval->Invalid("cant find asset create txid");
        if (!eval->GetTxUnconfirmed(assetid2,createTx,hashBlock) && assetid2 != zero)
            return eval->Invalid("cant find asset2 create txid");
    }
    else if (IsCCInput(tx.vin[0].scriptSig))
    ...
    else if (funcid != 'c')
    {
        ...
        if (!AssetExactAmounts(2, cp,inputs,starti,outputs,eval,tx,assetid))
    ...
    switch ( funcid )
    {
        ...
        case 'b': // buyoffer
            ...
            if ( !ConstrainVout(tx.vout[0],1,cp->unspendableCCaddr,0) )
        ...
        case 'o': // cancelbuy
            ...
            if ( (nValue= AssetValidateBuyvin(cp,eval,tmpprice,tmporigpubkey,CCaddr,origaddr,tx,assetid)) == 0 )
                return false;
            if ( !ConstrainVout(tx.vout[0],0,origaddr,nValue) )
        ...
        case 'B': // fillbuy:
            ...
            {
                if ( !ConstrainVout(tx.vout[2],1,CCaddr,0) )
                    return eval->Invalid("vout2 doesnt go to origpubkey fillbuy");
                if ( inputs != tx.vout[2].nValue+tx.vout[3].nValue )
                    return eval->Invalid("asset inputs doesnt match vout2+3 fillbuy");
            }
            if ( !ConstrainVout(tx.vout[2],1,CCaddr,inputs) )
                return eval->Invalid("vout2 doesnt match inputs fillbuy");
            if ( !ConstrainVout(tx.vout[1],0,0,0) )
                return eval->Invalid("vout1 is CC for fillbuy");
            if ( !ValidateBidRemainder(remaining_price,tx.vout[0].nValue,nValue,tx.vout[1].nValue,tx.vout[2].nValue,totalunits) )
                return eval->Invalid("mismatched remainder for fillbuy");
            if ( remaining_price != 0 && !ConstrainVout(tx.vout[0],1,cp->unspendableCCaddr,0) )
        ...
        case 's': // selloffer
            ...
            {
                ++preventCCvouts;
                if ( !ConstrainVout(tx.vout[0],1,(char *)cp->unspendableCCaddr,0) )
                    return eval->Invalid("mismatched vout0 AssetsCCaddr for selloffer");
                if ( tx.vout[0].nValue+tx.vout[1].nValue != inputs )
                    return eval->Invalid("mismatched vout0+vout1 total for selloffer");
            } else if ( !ConstrainVout(tx.vout[0],1,(char *)cp->unspendableCCaddr,inputs) )
                return eval->Invalid("mismatched vout0 AssetsCCaddr for selloffer");
        ...
        case 'x': // cancel
            ...
            if ( (assetoshis= AssetValidateSellvin(cp,eval,tmpprice,tmporigpubkey,CCaddr,origaddr,tx,assetid)) == 0 )
                return false;
            if ( !ConstrainVout(tx.vout[0],1,CCaddr,assetoshis) )
        ...
        case 'S': // fillsell
            ...
            if ( !ValidateAskRemainder(remaining_price,tx.vout[0].nValue,assetoshis,tx.vout[1].nValue,tx.vout[2].nValue,totalunits) )
                return eval->Invalid("mismatched remainder for fillask");
            if ( !ConstrainVout(tx.vout[1],1,0,0) )
                return eval->Invalid("normal vout1 for fillask");
            if ( !ConstrainVout(tx.vout[2],0,origaddr,0) )
                return eval->Invalid("normal vout1 for fillask");
            if ( remaining_price != 0 && !ConstrainVout(tx.vout[0],1,(char *)cp->unspendableCCaddr,0) )
        ...
        case 'E':
            ...
            //vout.n-1: opreturn [EVAL_ASSETS] ['E'] [assetid vin0+1] [assetid vin2] [remaining asset2 required] [origpubkey]
            // if ( !AssetExactAmounts(1, cp,inputs,1,outputs,eval,tx,assetid2) )
            ...
            //     else if ( tx.vout[3].scriptPubKey.IsPayToCryptoCondition() )
            //     {
            //         if ( !ConstrainVout(tx.vout[2],1,CCaddr,0) )
