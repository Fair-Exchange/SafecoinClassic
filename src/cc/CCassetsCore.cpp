...
bool ValidateBidRemainder(int64_t remaining_units,int64_t remaining_nValue,int64_t orig_nValue,int64_t received_nValue,int64_t paidunits,int64_t totalunits)
{
    ...
    if ( totalunits != remaining_units + paidunits )
    {
    ...
    if ( orig_nValue != remaining_nValue + received_nValue )
    {
...
bool ValidateAskRemainder(int64_t remaining_nValue,int64_t remaining_assetoshis,int64_t orig_assetoshis,int64_t received_assetoshis,int64_t paid_nValue,int64_t total_nValue)
{
    ...
    if ( total_nValue != remaining_nValue + paid_nValue )
    {
    ...
    if ( orig_assetoshis != remaining_assetoshis + received_assetoshis )
    {
...
bool ValidateSwapRemainder(int64_t remaining_price,int64_t remaining_nValue,int64_t orig_nValue,int64_t received_nValue,int64_t paidunits,int64_t totalunits)
{
    ...
    if ( totalunits != remaining_price + paidunits )
    {
...
bool DecodeAssetCreateOpRet(const CScript &scriptPubKey,std::vector<uint8_t> &origpubkey,std::string &name,std::string &description)
{
    ...
    if ( script != nullptr && vopret.size() > 2 && script[0] == EVAL_ASSETS && script[1] == 'c' )
        return E_UNMARSHAL(vopret,ss >> evalcode; ss >> funcid; ss >> origpubkey; ss >> name; ss >> description) != 0;
    return false;
}
...
uint8_t DecodeAssetOpRet(const CScript &scriptPubKey,uint256 &assetid,uint256 &assetid2,int64_t &price,std::vector<uint8_t> &origpubkey)
{
    ...
    price = 0;
    if ( script != nullptr && script[0] == EVAL_ASSETS )
    {
        funcid = script[1];
...
bool SetAssetOrigpubkey(std::vector<uint8_t> &origpubkey,int64_t &price,const CTransaction &tx)
{
    ...
    return !tx.vout.empty() && DecodeAssetOpRet(tx.vout.back().scriptPubKey,assetid,assetid2,price,origpubkey) != 0;
}

bool GetAssetorigaddrs(struct CCcontract_info *cp,char *CCaddr,char *destaddr,const CTransaction& tx)
{
    uint256 assetid,assetid2; int64_t price,nValue=0; uint8_t funcid; std::vector<uint8_t> origpubkey; CScript script;
    if ( tx.vout.empty() || (funcid= DecodeAssetOpRet(tx.vout.back().scriptPubKey,assetid,assetid2,price,origpubkey)) == 0 )
        return false;
    return GetCCaddress(cp,CCaddr,pubkey2pk(origpubkey)) && Getscriptaddress(destaddr,CScript() << origpubkey << OP_CHECKSIG);
}
...
int64_t IsAssetvout(int32_t maxAssetExactAmountDepth, struct CCcontract_info *cp, Eval* eval, int64_t &price,std::vector<uint8_t> &origpubkey,const CTransaction& tx,int32_t v,uint256 refassetid)
{
    ...
    if ( tx.vout[v].scriptPubKey.IsPayToCryptoCondition() ) // maybe check address too?
    {
        ...
        if ( (funcid= DecodeAssetOpRet(tx.vout[n-1].scriptPubKey,assetid,assetid2,price,origpubkey)) == 0 )
        {
        ...
        else if ( funcid == 'c' && refassetid == tx.GetHash() && v == 0) {
            ...
            return 0;
        else if ( funcid != 'E' && assetid == refassetid ) {
...
int64_t AssetValidateCCvin(struct CCcontract_info *cp,Eval* eval,char *CCaddr,char *origaddr,const CTransaction &tx,int32_t vini,CTransaction &vinTx)
{
    ...
    else if ( !eval->GetTxUnconfirmed(tx.vin[vini].prevout.hash,vinTx,hashBlock) )
    {
        int32_t z;
        for (z=31; z>=0; z--)
    ...
    else if ( !Getscriptaddress(destaddr,vinTx.vout[tx.vin[vini].prevout.n].scriptPubKey) || strcmp(destaddr,(char *)cp->unspendableCCaddr) != 0 )
    {
    ...
    else if ( !GetAssetorigaddrs(cp,CCaddr,origaddr,vinTx) )
...
int64_t AssetValidateBuyvin(struct CCcontract_info *cp,Eval* eval,int64_t &tmpprice,std::vector<uint8_t> &tmporigpubkey,char *CCaddr,char *origaddr,const CTransaction &tx,uint256 refassetid)
{
    ...
    if ( (nValue= AssetValidateCCvin(cp,eval,CCaddr,origaddr,tx,1,vinTx)) == 0 )
        return 0;
    else if ( !vinTx.vout[0].scriptPubKey.IsPayToCryptoCondition() )
    ...
    else
    {
        ...
        if ( !vinTx.vout.empty() && (funcid= DecodeAssetOpRet(vinTx.vout.back().scriptPubKey,assetid,assetid2,tmpprice,tmporigpubkey)) != 'b' && funcid != 'B' )
...
{
    CTransaction vinTx; int64_t assetoshis;
    fprintf(stderr,"AssetValidateSellvin\n");
    if ( AssetValidateCCvin(cp,eval,CCaddr,origaddr,tx,1,vinTx) == 0 )
        return 0;
    if ( (assetoshis= IsAssetvout(1, cp, nullptr, tmpprice,tmporigpubkey,vinTx,0,assetid)) == 0 )
...
{
    CTransaction vinTx; uint256 hashBlock,id,id2; bool flag; int32_t i,numvins,numvouts; int64_t assetoshis; std::vector<uint8_t> tmporigpubkey; int64_t tmpprice;
    numvins = tx.vin.size();
    ...
    for (i=starti; i<numvins; i++)
    {
        if ( (*cp->ismyvin)(tx.vin[i].scriptSig) )
        {
            ...
            if ( (eval != nullptr && !eval->GetTxUnconfirmed(tx.vin[i].prevout.hash,vinTx,hashBlock)) || (eval == nullptr && !myGetTransaction(tx.vin[i].prevout.hash, vinTx, hashBlock)) )
            {
                fprintf(stderr,"AssetExactAmounts() cannot read vintx i.%d starti.%d numvins.%d\n", i,starti,numvins);
                return eval != nullptr ? eval->Invalid("always should find vin, but didnt") : false;
            ...
            else if ( vinTx.vout[i].scriptPubKey.IsPayToCryptoCondition() && DecodeAssetOpRet(vinTx.vout.back().scriptPubKey,id,id2,tmpprice,tmporigpubkey) == 't' && id == assetid ) {
    ...
    flag = DecodeAssetOpRet(tx.vout.back().scriptPubKey,id,id2,tmpprice,tmporigpubkey) == 't' && id == assetid;
    ...
    {					     	  // 'false' means 'dont go deep' -- dimxy
        ...
        else if ( flag && tx.vout[i].scriptPubKey.IsPayToCryptoCondition() )
        {
